...
function fetchRadarImages(){
  // Check if radar container exists before trying to append
  var radarContainer = document.getElementById('radar-container');
  if (!radarContainer) return;

  radarContainer.innerHTML = ""; // Clear old radar
  ...
  if(alertsActive){
    var zoomedContainer = document.getElementById('zoomed-radar-container');
    if (zoomedContainer) {
        zoomedContainer.innerHTML = "";
        zoomedRadarImage = document.createElement("iframe");
        // ... (settings for zoomed radar)
        zoomedContainer.appendChild(zoomedRadarImage);
    }
  }
...
/**
 * AUTO-REFRESH ENGINE 
 * Triggers the entire chain every 60 seconds
 */
setInterval(function() {
    console.log("Minute update: Refreshing weather data waterfall...");
    fetchCurrentWeather();
}, 60000); 
